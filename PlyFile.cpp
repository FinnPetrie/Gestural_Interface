...
PlyFile::PlyFile(std::vector<Vertex> vertices) : points_(vertices){
...
std::vector<Vertex> PlyFile::getPoints(){
	return points_;
}
...
Vertex PlyFile::getRandomPoint(){
	std::cout<<"Getting random" <<std::endl;
	Vertex v = points_[rand()%points_.size()];
	v.print();
...
Vertex PlyFile::getPointAt(int i){
